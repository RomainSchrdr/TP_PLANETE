...
test_result t_vec2_rotate_0() {
    double r_x[] = {2.682078, 4.048477, -3.716744, -9.006930};
    double r_y[] = {-2.462795, 6.942560, 4.632258, -1.506259};

    bool passed = true;
    for (uint32_t i = 0; i < nb_tests; i++) {
        double theta = v_x[i];
        vec2 u = vec2_create(u_x[i], u_y[i]);
        vec2 r = vec2_rotate(theta, u);
        if (!(dbl_eq(r.x, r_x[i]) && dbl_eq(r.y, r_y[i]))) {
            passed = false;
            break;
        }
    ...
    return (test_result){.passed = passed, .name = "Test vec2_rotate 0"};
}
...
const unit_test_t tests[] = {t_vec2_create_0, t_vec2_create_zero_0, t_vec2_add_0,
                             t_vec2_sub_0, t_vec2_mul_0, t_vec2_rotate_0, t_vec2_dot_0,
                             t_vec2_norm_sqr_0, t_vec2_norm_0, t_vec2_normalize_0,
                             t_vec2_is_approx_equal_0, t_vec2_to_coordinates_0};
